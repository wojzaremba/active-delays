initialize();
load([getenv('data_path'), 'processed08_pca.mat'], 'X', 'Y');
setenv('log_level', '2');
...
windowRange = 0:5;
while (true)
    idx = getCacheIndex(struct('window', windowRange, 'fold', 1:folds, 'C', C), cacheName);
    if idx.done, break; end;
    [ XTrain, YTrain, XTest, YTest ] = drawData(X, Y, idx.fold, folds);
    if (idx.window ~= 0)        
        model_ = retriveCacheValue(struct('window', idx.window - 1, 'fold', idx.fold, 'C', idx.C), cacheName);
        if (~model_.cached)
            continue;
        end
        model_ = model_.model;
...
for windowIdx = 1:size(windowRange, 2)
    for fold = 1:folds
        value = retriveCacheValue(struct('window', windowRange(windowIdx), 'fold', fold, 'C', C), cacheName);
        acc(fold, windowIdx) = value.acc;    
    end
end

[~, p] = ttest2(acc(:, 1), acc(:, 4));
