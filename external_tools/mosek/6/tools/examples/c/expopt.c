  ...
  */
{
  ...
  /* clear expopttask */
  {
    ...
    if (r == MSK_RES_OK)
      r = MSK_remove(expopttask,MSK_ACC_VAR,itmp,delsub);

    if (r == MSK_RES_OK)
      r = MSK_remove(expopttask,MSK_ACC_CON,itmp2,delsub); 
  ...
  if (solveform >=0)  /* If the dual formulation was chosen */
  {
    ...
    if (r == MSK_RES_OK)
      r = MSK_append(expopttask,MSK_ACC_VAR,numter);
    
    if (r == MSK_RES_OK)
      r = MSK_append(expopttask,MSK_ACC_CON,numvar+1);
    ...
    while (itmp < numanz && r == MSK_RES_OK)
    {
      ...
      if (r == MSK_RES_OK)
        r = MSK_putavec(expopttask,MSK_ACC_VAR,pos[curterm],nz,ibuf,rbuf);   
    }
  ...
  { 
    ...
    if (r == MSK_RES_OK)
      r = MSK_append(expopttask,MSK_ACC_VAR,numvar + numter);
    
    if (r == MSK_RES_OK)
      r = MSK_append(expopttask,MSK_ACC_CON,numcon + numter);
    ...
    itmp=0;
    {
      ...
      for (termindex=0;termindex<numter;++termindex)
      {
        if (subk[itmp] != termindex)
        {
          ...
          if (r == MSK_RES_OK)
            r = MSK_putavec(expopttask,MSK_ACC_CON,termindex+numcon,nz,ibuf,rbuf);
