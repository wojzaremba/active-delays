...
static void scgrdobjstruc(nlhand_t nlh,
                          int      *nz,
                          int      *sub)
/* Purpose: To compute number of nonzeros and sparsity
...
static int evalopr(int    opr,
                   ...
                   double *grdfxj,
                   double *hesfxj)
/* Purpose: To evaluate an operator and its derivatives.
...
static int scobjeval(nlhand_t nlh,
                     ...
                     int      *grdsub,
                     double   *grdval)
     /* Purpose: To compute number objective value and the gradient.
      */
{
  int    j,k,
         *zibuf;
                        ...
                        char      filename[])
{
  ...
  nlh = (nlhand_t) sch;
  l   = strlen(filename);
  fn  = MSK_calloctask(task,l+5,sizeof(char));
  if ( fn )
  {
...

    {
      ...
      if ( r==MSK_RES_OK )
      {
        ...
        if ( f )
        {
          ...
          if ( numopro )
          {
            opro  = MSK_calloctask(task, numopro, sizeof(int));
            oprjo = MSK_calloctask(task, numopro, sizeof(int));
            oprfo = MSK_calloctask(task, numopro, sizeof(double));
            oprgo = MSK_calloctask(task, numopro, sizeof(double));
            oprho = MSK_calloctask(task, numopro, sizeof(double));
          ...
          { 
            ...
            if ( numoprc )
            {
              oprc  = MSK_calloctask(task,numoprc,sizeof(int));
              opric = MSK_calloctask(task,numoprc,sizeof(int));
              oprjc = MSK_calloctask(task,numoprc,sizeof(int));
              oprfc = MSK_calloctask(task,numoprc,sizeof(double));
              oprgc = MSK_calloctask(task,numoprc,sizeof(double));
              oprhc = MSK_calloctask(task,numoprc,sizeof(double));
