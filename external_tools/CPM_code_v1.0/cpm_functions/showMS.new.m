...
function H = showMS(qmzD,merged,myWindow,lineMarkerPos,...
    timeRatio,SHOW_DIFF,keepScans,plotLog,numDig,LOW,colorScheme)
...
theseMaps{6} = {'whiteToRed3' 'whiteToGreen3'};
...
for ii=1:length(qmzD)
    ...
    %% dynamic range of the data and tons of garbage
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %% scale the plot data to have min=0 and max=10^7, in a
    %weird way
    if (0)%newWay
        %% get rid outliers, and rescale the data
        %% so that they have same dynamic range
        tempDat = plotData(find(plotData));
        tempSort = sort(tempDat(:));
        tempNum = length(tempSort);
        mymin=0; mymax=1e+08;
        clipmax=tempSort(round(0.99*tempNum));
        clipmin=mymin;
        newPlotData=rescaledata(plotData,mymin,mymax,clipmin,clipmax);
        %% find out where the background is and set everything
        %below it to zero so that there isn't too much crap visible
        tempDat = newPlotData(find(newPlotData));
        tempSort = sort(tempDat(:));
        fracEmpty=0.98;
        backValue = tempSort(round(fracEmpty*tempNum));
        %figure,plot(tempSort,'-'); hold on, plot(backChangeInd,garb,'r+');
        %figure,plot(tempSort,'-'); hold on, plot(backChangeInd,tempSort(backChangeInd),'r+');
        numBelow=length(find(newPlotData<backValue));
        display(['Frac below threshold: ' num2str(numBelow/prod(size(newPlotData)))]);
        newPlotData(find(newPlotData<backValue))=0; %backValue;

        plotData=newPlotData;
        %length(find(plotData==0))/prod(size(plotData))
        ...
        if ~isempty(lineMarkerPos)
            ...
            for jj=1:length(ms)
                ...
                if (timeRatio>1)
                    ...
                    text(myLineX(2)+4+10*(ii-1),myLineY(1), guideLabels(jj),'Color', mycolours2{ii},'FontSize',12);
                else
