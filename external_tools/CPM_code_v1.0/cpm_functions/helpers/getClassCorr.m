...
if strcmp(statType,'welchT')
    ...
    minThresh=1e-2;
    if any(v1(:)<-minThresh) || any(v2(:)<-minThresh)
        min(v1(:))
        min(v2(:))
...
%     myMean=(m1+m2)/2;
%     stat2 = abs(m1-m2)./myMean;  
%     stat = stat.*stat2;
    
    
    %return;
elseif strcmp(statType,'relDiff')   
    %% renormalize each set of measurements by the mean
    myMean=(m1+m2)/2;
    stat = (m1-m2)./myMean;    
else
    error('need to select welchT or relDiff for statType');
end
...
if 1
    %% absolute differencd
    badInd = find(abs(m1-m2)<minDiff);
    stat(badInd)=0;

    %relative difference (minDiff \in [0 Inf])
    relDiff = (m1+m2)/2*minRelDiff;
    badInd2 = find(abs(m1-m2)<relDiff);
    stat(badInd2)=0;
end
...
if 0
    str=['minDiff=' num2str(minDiff,3) ', minRelDiff=' num2str(minRelDiff)];
    figure, showStats(stat,str);
end
