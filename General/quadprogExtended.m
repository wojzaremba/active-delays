function [x,fval,exitflag,output,lambda]=quadprogExtended(H,f,A,b,B,c,l,u,x0,options)
nrproc = feature('numCores');
nrproc = min(nrproc, 6);
exitflag = -10;
eigs = eig(H);


e = min(min(real(eigs)), min(imag(eigs)));
if ((e < 0) && (e > -0.00000001))
    H = H + eye(size(H,1)) * abs(e) * 1.5;
end

if (min(real(eigs)) < 1e-10)
    H = H + eye(size(H,1)) * 1e-10;
end
...
param.MSK_IPAR_INTPNT_NUM_THREADS = nrproc;
